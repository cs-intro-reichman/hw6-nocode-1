...
public class Runigram {
	...
	public static Color[][] flippedVertically(Color[][] image) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				flipped[height - 1 - i][j] = image[i][j];
			}
		}
		return flipped;
	}
	...
	private static Color luminance(Color pixel) {
		...
		int lumInt = (int) lum;
		return new Color(lumInt, lumInt, lumInt);
	}
	/**
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int sourceX = (int) (j * sourceWidth / (double) width);
				int sourceY = (int) (i * sourceHeight / (double) height);
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		for (int i = 0; i < n; i++) {
			...
			Color[][] blendedImage = blend(source, scaled(target, source[0].length, source.length), alpha);
